class HardWorkerJob < ApplicationJob
    ...
    def perform(*args)
      ...
      # #puts binance,"----------binance--------"
      # @gate_price = JSON.parse(gate.body)["btc_usdt"]["last"]
      # puts @gate_price,"-------gate-----"
      begin
        gate            =   RestClient.get "https://data.gateio.co/api2/1/tickers"
      ...
      @welcome.save
      ActionCable.server.broadcast("public",{success: true,code:'200',action: "btcusdt",data: Welcome.list_limit_6})
      ActionCable.server.broadcast("public",{success: true,code:'200',action: "ethusdt",data: Welcome.list_limit_6('ethusdt')})
    end
end
